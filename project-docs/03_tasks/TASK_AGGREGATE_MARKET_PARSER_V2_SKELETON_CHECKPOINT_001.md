...
```text
TASK_AGGREGATE_MARKET_PARSER_V2_SKELETON_CHECKPOINT_001
```
...
```text
Aggregate market-parser-v2 skeleton checkpoint scope
```
...
```text
VALUE: maximum
OVERRIDE_REASON: Aggregate checkpoint covers accepted implementation skeleton, audit records, and orchestrator-owned runtime route records.
```
...
```text
- local-only Git checkpoint for accepted market-parser-v2 skeleton and runtime records
```
...
```text
Provide explicit aggregate checkpoint scope for the accepted market-parser-v2 skeleton implementation, audit result, and orchestrator-owned runtime records.
```
...
```text
- validate changed-file scope for accepted market-parser-v2 skeleton bundle
...
```text
- accepted market-parser-v2 skeleton audit pass
```
