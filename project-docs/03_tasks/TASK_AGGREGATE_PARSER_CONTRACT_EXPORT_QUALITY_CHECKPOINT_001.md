...
```text
TASK_AGGREGATE_PARSER_CONTRACT_EXPORT_QUALITY_CHECKPOINT_001
```
...
```text
Aggregate parser contract export quality checkpoint scope
```
...
```text
VALUE: maximum
OVERRIDE_REASON: Aggregate checkpoint covers accepted contract/export/quality implementation, audit/test records, and orchestrator-owned runtime route records.
```
...
```text
- local-only Git checkpoint for accepted parser contract/export/quality implementation and runtime records
```
...
```text
Provide explicit aggregate checkpoint scope for accepted parser contract/export/quality implementation, audit and test results, and orchestrator-owned runtime records.
```
...
```text
- validate changed-file scope for accepted parser contract/export/quality bundle
...
```text
- accepted parser contract/export/quality test pass
```
...
- secret scan passes
- push is requested by project owner after local checkpoint
```
