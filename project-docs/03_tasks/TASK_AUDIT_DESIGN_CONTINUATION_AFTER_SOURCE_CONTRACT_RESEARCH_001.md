...
```text
Audit design continuation after source contract research
```
...
```text
VALUE: maximum
OVERRIDE_REASON: Audit must verify source-contract evidence, task schema validity, forbidden path scope, and designer-to-auditor workflow preservation.
```

## DEPENDENCIES

```text
- TASK_DESIGN_CONTINUATION_AFTER_SOURCE_CONTRACT_RESEARCH_001 returned pass
```
...
- task packet/proposal schema validation status
- audited research evidence status
- forbidden path and runtime mutation status
...
```text
Audit the bounded architecture and downstream task artifacts produced from audited WB/Ozon source-contract research.
```
...
```text
- verify design used only REQUIRED_DOCS and READ_INPUTS source-contract evidence
- verify changed files are within design task ALLOWED_FILE_CHANGES
- verify no forbidden paths were modified
- verify finalized contracts cite audited WB/Ozon research results
- verify unresolved formulas, thresholds, and business decisions remain gaps or proposals
- verify every changed task-like artifact is classified and schema-valid
- verify provider migration tasks are bounded and dependency-gated
- verify designer -> auditor transition is preserved
```

## SCOPE_OUT

```text
- do not inspect WB or Ozon source projects directly
...
```text
- designer RESULT for TASK_DESIGN_CONTINUATION_AFTER_SOURCE_CONTRACT_RESEARCH_001
```
...
```text
- design evidence lists read documents, changed files, and audited research references
- all changed paths are within the design task ALLOWED_FILE_CHANGES
- no forbidden path mutation is found
- source-contract facts come only from audited WB/Ozon research RESULTS
- unresolved facts are gaps, limitations, or research dependencies
- dispatchable task packets pass schema validation
- non-dispatchable proposals pass proposal validation and remain non-dispatchable
...
- unaudited source facts or guessed business formulas must fail audit
```
...
```text
This audit is the mandatory designer -> auditor transition for TASK_DESIGN_CONTINUATION_AFTER_SOURCE_CONTRACT_RESEARCH_001.
```
