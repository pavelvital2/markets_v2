...
```text
Audit parser contract export and quality enforcement
```
...
```text
VALUE: high
OVERRIDE_REASON: Audit covers provider identity, export safety, data quality, and security constraints.
```

## DEPENDENCIES

```text
- TASK_DEV_PARSER_CONTRACT_EXPORT_QUALITY_001 returned pass or fail
```
...
- contract/export behavior evidence
...
```text
Audit common parser schema, export, data-quality, and forbidden-artifact enforcement before provider migration depends on it.
```
...
- verify source projects were not modified
- verify common schema and provider identity rules
- verify schema_version and unsupported-version behavior
- verify export manifest/checksum behavior
- verify forbidden artifact exclusion
- verify partial/failure data-quality preservation
...
- do not define business formulas
...
```text
- developer RESULT for TASK_DEV_PARSER_CONTRACT_EXPORT_QUALITY_001
```
...
```text
- implementation satisfies contract/export/data-quality/security architecture
- forbidden path and secret checks pass
- tests are sufficient for synthetic contract/export behavior or limitations are explicit
...
```text
- contract/export implementation may pass unit tests but still need provider integration testing
```
...
```text
On audit pass, orchestrator may dispatch TASK_TEST_PARSER_CONTRACT_EXPORT_QUALITY_001.
```
