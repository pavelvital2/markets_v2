...
```text
Audit WB source contract research
```
...
```text
VALUE: high
OVERRIDE_REASON: Research dependency audit must verify bounded source use and secret exposure.
```

## DEPENDENCIES

```text
- TASK_RESEARCH_WB_SOURCE_CONTRACTS_001 returned pass
```
...
```text
Audit WB source-contract research for bounded-source compliance, evidence quality, and secret exposure.
```
...
```text
Independently audit the WB source-contract research result before it may be used by designer continuation.
```
...
- research changed files are limited to TASK_RESEARCH_WB_SOURCE_CONTRACTS_001 ALLOWED_FILE_CHANGES
- report sources are within task REQUIRED_DOCS and ALLOWED_SOURCES
- no forbidden source content or secret values are copied
- unresolved findings are recorded rather than guessed over
```
...
```text
This audit does not authorize designer continuation until it returns pass and orchestrator records the result.
```
