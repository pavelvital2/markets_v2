...
INSTRUCTION_FOR_ORCHESTRATOR: Dispatch exactly one requirements_analyst task with reasoning_effort xhigh because role default resolves to maximum.
```
...
```text
Dispatch exactly one requirements_analyst task with reasoning_effort xhigh because role default resolves to maximum.
```
