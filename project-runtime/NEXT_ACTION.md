...
INSTRUCTION_FOR_ORCHESTRATOR: Dispatch exactly one developer task with reasoning_effort xhigh; allowed source reads are restricted to TASK_DEV_OZON_PROVIDER_MIGRATION_001 ALLOWED_SOURCES.
```
...
```text
Dispatch exactly one developer task with reasoning_effort xhigh; allowed source reads are restricted to TASK_DEV_OZON_PROVIDER_MIGRATION_001 ALLOWED_SOURCES.
```
